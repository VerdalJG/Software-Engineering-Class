...
void CORE_RenderCenteredSprite(const vec2 &position, const vec2 &size, GLuint texid)
{
  vec2 p0 = position - (size * .5f);
  vec2 p1 = position + (size * .5f);
...
void CORE_RenderCenteredRotatedSprite(const vec2 &position, const vec2 &size, float angle, GLuint texid, rgba_t color)
{
